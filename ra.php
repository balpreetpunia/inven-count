...
if ($model != ''){
    ...
    if($count > 0){
        if($qty == '') {
            $sql = "UPDATE inventory_ra SET counted = counted +1 , Last_Updated = '$time' WHERE model = '$model'";
            $dbh->exec($sql);
            $error = 0;
        }
        else{
            $sql = "UPDATE inventory_ra SET counted = counted +$qty , Last_Updated = '$time' WHERE model = '$model'";
            $dbh->exec($sql);
            $error = 0;
        }
    }
